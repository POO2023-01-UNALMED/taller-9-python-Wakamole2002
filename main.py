from tkinter import StringVar, Tk, Button, Entry, Label
...
def tecla(t):
    global var
    var= var+str(t) # anida cada tecla presionada por el usuario
    entrada.set(var)
    
def result():
    global var 
    try:
        a=str(eval(var)) # evualua el string y realiza la operacion dependiendo de los caracteres 
        entrada.set(a) 
    except:
        entrada.set("")   
    var = "" 
...
pantalla = Entry(root, width=41, bg="black", fg="white", borderwidth=0, font=("arial", 20, "bold"))
...
#con los command se envia una funcion lambda que envia la tecla seleccionada
